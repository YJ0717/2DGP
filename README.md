...
무기와 마법 공격은 플레이어의 입력에 따라 활성화되며, 적과의 충돌(예정)을 처리할 수 있습니다. <br/>

플레이어와 UI <br/>
플레이어의 상태 변화(예: HP 감소)에 따라 UI가 업데이트됩니다 (예정). <br/>
...
Monster: 체력, 공격 패턴을 포함하고 있습니다.(추후 업데이트) <br/> 

NPC: 대화 및 기본아이템 지급 기능을 하고있습니다.(추후 업데이트) <br/>

Item: 회복, 장비 등 소유자 관리할 수 있는 아이템 클래스 입니다. (추후 업데이트) <br/>
...
PhysicsController: 캐릭터와 환경 간 충돌 관리 클래스 입니다.(추후 업데이트) <br/>

SoundController: 효과음 및 배경음 관리를 할 수 있는 클래스입니다.(추후 업데이트 )  <br/>
...
2주차 스킬과 hud 연동 구현 을 제외한 2주차 전부 구현 90% <br/>

3주차 1차스킬 및 쿨타임 구현완료 2차스킬 및 충돌처리 미구현 충돌처리는 몬스터 구현할때 동시에 진행할 예정  50% <br/>

4주차 1스테이지 구현 완료 사냥터인 2~4스테이지 및 마을 npc 미구현 30% <br/>
